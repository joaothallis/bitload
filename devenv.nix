...
}:
{
  ...
  processes = {
    bitcoin-node-1.exec = "bitcoind -regtest -datadir=$DEVENV_STATE/bitcoin-node-1 -port=18444 -rpcport=18443 -printtoconsole";
    bitcoin-node-1.restart.on = "never";
    bitcoin-node-1.process-compose.availability.restart = "no";
    bitcoin-node-2.exec = "bitcoind -regtest -datadir=$DEVENV_STATE/bitcoin-node-2 -port=18445 -rpcport=18446 -connect=127.0.0.1:18444 -printtoconsole";
